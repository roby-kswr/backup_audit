...
class Laporan_pelaksanaan_progja_log extends Controller
{
   ...
   private $model      = "laporan_pelaksanaan_progja_log_model";

	private $title      = "Laporan Pelaksanaan Kerja";

	private $curl       = BASE_URL."laporan_pelaksanaan_progja_log";
   ...
   function index()
   {
      ...
      $data['ttd']         = $model->ttd();
      $template            = $this->loadView('laporan_pelaksanaan_progja_log_view');
       ...
       // $pdf->Cell(260,1,'','B',0,'L');
        ...
        if ($satker == 00) {
           $query = $model->query("SELECT id, kd_bidang,nm_bidang, FORMAT(jml_pagu,0) AS jumlah, urskmpnen,thang,kdsatker,nm_satminkal,nmakun FROM tbidang a LEFT JOIN( SELECT id, wasgiat, jml_pagu,urskmpnen,thang,kdsatker,kdakun  FROM dja_pagu ) AS b ON a.wasgiat = b.wasgiat
             LEFT JOIN (SELECT kdakun AS akun ,nmakun FROM dja_akun) AS d ON b.kdakun = d.akun
             LEFT JOIN (SELECT nm_satminkal,kd_satminkal FROM tsatminkal) AS c ON  b.kdsatker = c.kd_satminkal WHERE a.kd_bidang  = 06 AND b.thang = '$tahun' GROUP BY b.kdakun ORDER BY nm_satminkal ASC ");
        ...
        foreach ($query as $row){
          ...
          $query = $model->query("SELECT id, kd_bidang,nm_bidang, FORMAT(jml_pagu,0) AS jumlah, urskmpnen,thang,kdsatker,nm_satminkal,nmakun FROM tbidang a LEFT JOIN( SELECT id, wasgiat, jml_pagu,urskmpnen,thang,kdsatker,kdakun  FROM dja_pagu ) AS b ON a.wasgiat = b.wasgiat
             LEFT JOIN (SELECT kdakun AS akun ,nmakun FROM dja_akun) AS d ON b.kdakun = d.akun
             LEFT JOIN (SELECT nm_satminkal,kd_satminkal FROM tsatminkal) AS c ON  b.kdsatker = c.kd_satminkal WHERE a.kd_bidang  = 06 AND c.kd_satminkal = '$satker' AND b.thang = '$tahun' GROUP BY b.kdakun ORDER BY nm_satminkal ASC");
